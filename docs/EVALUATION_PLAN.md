...
| G-10 | **Slide 6 names "Qwen3 language models"** without saying vision-capable | Pipeline requires image input; mock-ups name Gemma 3 4B / Qwen2-VL 2B; docs TBD. A vision build exists (`qwen3-vl` 2B/4B/8B on Ollama ≥ 0.12.7), so this is a wording and pinning issue, not a blocker | A text-only build cannot satisfy FR-14 | Name the exact `-vl` tag on the slide and in config; VC-05 decides the model |
